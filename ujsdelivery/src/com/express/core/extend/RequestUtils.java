...
/*    public static String getRemoteAddr(HttpServletRequest req) {
        ...
        if (Strings.isNotBlank(ip)) {
            String[] ips = Strings.splitIgnoreBlank(ip, ",");
            if (ips != null) {
                for (String tmpip : ips) {
                    if (Strings.isBlank(tmpip))
                        continue;
                    tmpip = tmpip.trim();
                    if (isIPAddr(tmpip)
                        && !tmpip.startsWith("10.")
    ...
    public static Cookie getCookie(HttpServletRequest request, String name) {
        ...
        for (Cookie ck : cookies) {
            if (Strings.equalsIgnoreCase(name, ck.getName()))
                return ck;
        }
        return null;
    }
    ...
    public static String getDomainOfServerName(String host) {
        if (isIPAddr(host))
            return null;
        String[] names = Strings.splitIgnoreBlank(host, ".");
    ...
    public static boolean isIPAddr(String addr) {
        if (Strings.isEmpty(addr))
            return false;
        String[] ips = Strings.splitIgnoreBlank(addr, ".");
        if (ips.length != 4)
            return false;
        try {
