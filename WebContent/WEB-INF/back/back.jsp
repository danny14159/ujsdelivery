...
<body>
<div class="alert alert-danger" role="alert" style="display: none;" id="newOrderMsg">您有新的订单，请注意查收。<!-- 上一次刷新时间：<span class="last-refresh"></span> -->
	<button onclick="location.reload();" class="btn btn-default btn-xs">点击刷新</button>
</div>
...
<c:if test="${type eq '代取快递'}">
	...
	<c:if test="${me.type == 'P' }">
	<a class="btn btn-default" href="/app/back/charge" >积分管理</a>
	</c:if>
<c:if test="${isManager }">	
...
//				printOrder($(this).closest('.list-item'));
	...
	function onPrintOrders(){
		...
		imgLogoOrder.src="/static/img/ali.png";
